			...
			<c:if test="${!empty authInfo }">
				...
				<c:forEach var="jGrp" items="${joinGrp }">
					<div class="mainGroups">
						...
						<span id="groupName">
							그룹명 : 
							<a href="<c:url value='/group/${jGrp.grpName }' />" id="groupName2">${jGrp.grpName }</a>
						</span>
							<c:if test="${authInfo.userNick==jGrp.grpLeader  }">
								<form name="modifyForm" method="post" action="modifyGroupInfo" style="display: inline;">
								...
								<input type="submit" style="float: right; margin-right: 15px; margin-top: 10px;" value="수정하기" />							
								</form>
							</c:if>
						</div>
						<div id="mainGrp2">
